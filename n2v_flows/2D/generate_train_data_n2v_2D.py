...
def load_imgs_from_directory(data_dir: str,
    ...
    imgs = datagen.load_imgs_from_directory(directory=data_dir,
                                            filter=filter,
                                            dims="YX")
    ...
    for img in imgs:
        if len(img.shape) != 4:
            logger.error(f"Loaded image shape is f{img.shape}.")
...
with Flow("N2V Data Generation [2D - 2D]",
    ...
    results = add_to_slurm_flow_run_table_task(prefect_context=context_dict,
                                               slurm_info_dict=slurm_info,
